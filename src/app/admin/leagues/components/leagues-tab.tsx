...
import { LeagueItem, PageResponse } from "@/services/admin-metadata-service";

interface LeaguesTabProps {
  leaguesData: PageResponse<LeagueItem> | undefined;
  isLoading: boolean;
  isAdmin: boolean;
  searchVal: string;
    ...
    <div className="space-y-6">
      ...
      {isLoading ? (
        <div className="flex py-12 justify-center">
          <p className="text-white/40 text-sm">Đang tải danh sách giải đấu...</p>
        </div>
      ) : (
        <div className="overflow-x-auto rounded-xl border border-white/10 bg-[#0B0F19]">
          <table className="w-full border-collapse text-left text-sm">
            <thead className="bg-white/5 border-b border-white/10">
              <tr>
                <th className="px-6 py-4 font-semibold text-white/60">Logo</th>
                <th className="px-6 py-4 font-semibold text-white/60">Tên Giải Đấu</th>
            ...
            <tbody className="divide-y divide-white/10">
              {leaguesData?.items.length === 0 ? (
                ...
                leaguesData?.items.map((l: LeagueItem) => (
                      ...
                      {l.logoUrl ? (
                        <>
                          ...
                          <img
                            src={l.logoUrl}
                            alt={l.leagueName}
                            className="h-8 w-8 object-contain filter drop-shadow-[0_2px_4px_rgba(255,255,255,0.1)]"
                            onError={(e) => {
                        ...
                        <span className="text-xs text-white/20">Không có logo</span>
                      )}
                    </td>
                    <td className="px-6 py-4 font-semibold text-white">{l.leagueName}</td>
                    <td className="px-6 py-4 font-mono text-white/60">{l.leagueSlug || "-"}</td>
