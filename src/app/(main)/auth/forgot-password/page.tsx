    ...
    <GuestGuard>
      <div className="flex min-h-[calc(100vh-80px)] items-center justify-center p-6">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-5">
            ...
            <div className="relative">
              ...
              <input
                ...
                className={`w-full rounded-xl border py-3 pl-10 pr-4 text-sm text-white placeholder-white/40 outline-none transition focus:bg-white/10 ${
                  errors.email ? "border-red-500 bg-red-500/10" : "border-white/10 bg-white/5 focus:border-[#00FF85]"
                }`}
              />
            </div>
            {errors.email && <p className="pl-1 text-xs font-medium text-red-400">{errors.email.message}</p>}
          </div>
          ...
          >
            {isSubmitting ? <Loader2 className="animate-spin" size={20} /> : "GỬI EMAIL KHÔI PHỤC"}
          </button>
        </form>
